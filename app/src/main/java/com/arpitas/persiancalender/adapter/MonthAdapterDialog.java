...
public class MonthAdapterDialog extends RecyclerView.Adapter<MonthAdapterDialog.ViewHolder> {
    ...
    public MonthAdapterDialog(Context context, MonthFragmentDialog monthFragment, List<DayEntity> days) {
        ...
        isEnglish = new SharedPrefManager(context).getLanguage().equalsIgnoreCase(SharedPrefManager.EN);
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_day_dialog, parent, false));
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        if (!isPositionHeader(position)) {
            if (position - 14 - days.get(0).getDayOfWeek() >= 0) {
                ...
                holder.num.setTextSize(isEnglish ? ApplicationContexts.GlobalContext.getResources().getInteger(R.integer.txt2) : ApplicationContexts.GlobalContext.getResources().getInteger(R.integer.txt1));
                holder.num.setTextColor(days.get(position - 14 - days.get(0).getDayOfWeek()).isHoliday() ? ContextCompat.getColor(context, colorHoliday.resourceId): ContextCompat.getColor(context, R.color.black));
            ...
            if (position >= 0 && position < 7) {
                ...
                holder.num.setTextColor(ContextCompat.getColor(context, R.color.black));
                holder.num.setTextSize(ApplicationContexts.GlobalContext.getResources().getInteger(R.integer.txt3));
                holder.today.setVisibility(View.GONE);
                holder.selectDay.setVisibility(View.GONE);
                holder.event.setVisibility(View.GONE);
                holder.num.setVisibility(View.VISIBLE);
